...
vec4 lighting_equation_textured(in vec3 P_EC, in vec3 N_EC, in vec4 base_color) {
	...
	if (u_wc_tog_light_flag == true)
	{
		for (int i = 0; i < NUMBER_OF_LIGHTS_SUPPORTED; i++) {
			...
			if (local_scale_factor > zero_f) {				
				...
				tmp_float = max(zero_f, dot(N_EC, L_EC)); 
				if (tmp_float > zero_f) {  
					local_color_sum += u_wc_tog_light[i].diffuse_color*base_color*tmp_float;
	...
	if (u_ec_tog_light_flag == true && u_ec_tog_light.light_on)
	{
		...
		if (local_scale_factor > zero_f) {				
			...
			tmp_float = max(zero_f, dot(N_EC, L_EC)); 
			if (tmp_float > zero_f) {  
				local_color_sum += u_ec_tog_light.diffuse_color*base_color*tmp_float;
	...
	if (u_mc_tog_light_flag == true && u_mc_tog_light.light_on)
	{
		...
		if (local_scale_factor > zero_f) {				
			...
			tmp_float = max(zero_f, dot(N_EC, L_EC)); 
			if (tmp_float > zero_f) {  
				local_color_sum += u_mc_tog_light.diffuse_color*base_color*tmp_float;
