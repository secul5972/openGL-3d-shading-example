...
uniform LIGHT u_tog_light[NUMBER_OF_LIGHTS_SUPPORTED];
...
vec4 lighting_equation(in vec3 P_EC, in vec3 N_EC) {
	...
	{
		for (int i = 0; i < NUMBER_OF_LIGHTS_SUPPORTED; i++) {
			if (!u_tog_light[i].light_on) continue;

			local_scale_factor = one_f;
			if (u_tog_light[i].position.w != zero_f) { // point light source
				L_EC = u_tog_light[i].position.xyz - P_EC.xyz;

				if (u_tog_light[i].light_attenuation_factors.w  != zero_f) {
					vec4 tmp_vec4;
					...
					tmp_vec4.y = sqrt(tmp_vec4.z);
					tmp_vec4.w = zero_f;
					local_scale_factor = one_f/dot(tmp_vec4, u_tog_light[i].light_attenuation_factors);
				}

				L_EC = normalize(L_EC);

				if (u_tog_light[i].spot_cutoff_angle < 180.0f) { // [0.0f, 90.0f] or 180.0f
					float spot_cutoff_angle = clamp(u_tog_light[i].spot_cutoff_angle, zero_f, 90.0f);
					vec3 spot_dir = normalize(u_tog_light[i].spot_direction);
					
					tmp_float = dot(-L_EC, spot_dir);
					if (tmp_float >= cos(radians(spot_cutoff_angle))) {
						tmp_float = pow(tmp_float, u_tog_light[i].spot_exponent);
					}
					else 
						tmp_float = zero_f;
					local_scale_factor *= tmp_float;
				...
				L_EC = normalize(u_tog_light[i].position.xyz);
			}	

			if (local_scale_factor > zero_f) {				
			 	vec4 local_color_sum = u_tog_light[i].ambient_color * u_material.ambient_color;

				tmp_float = max(zero_f, dot(N_EC, L_EC));  
				if (tmp_float > zero_f) {  
					local_color_sum += u_tog_light[i].diffuse_color*u_material.diffuse_color*tmp_float;
				
					vec3 H_EC = normalize(L_EC - normalize(P_EC));
					tmp_float = max(zero_f, dot(N_EC, H_EC)); 
					if (tmp_float > zero_f) {
						local_color_sum += u_tog_light[i].specular_color
					                       *u_material.specular_color*pow(tmp_float, u_material.specular_exponent);
					}
				}
				color_sum += local_scale_factor*local_color_sum;
